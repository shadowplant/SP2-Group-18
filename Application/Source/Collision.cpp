...
bool Collision::raysphere(float xc, float yc, float zc, float xd, float yd, float zd, float xs, float ys, float zs, float r, float* dist, Vector3* collpoint)
{
...
float Collision::trianglearea(Vector3 p1, Vector3 p2, Vector3 p3)
{
...
float Collision::pointdistace(Vector3 p1, Vector3 p2)
{
	Vector3 vec(p2.x - p1.x, p2.y - p1.y, p2.z - p1.z);
	return (sqrt(vec.x * vec.x + vec.y * vec.y + vec.z * vec.z));
}

float Collision::pointdistacesquare(Vector3 p1, Vector3 p2)
{
	Vector3 vec(p2.x - p1.x, p2.y - p1.y, p2.z - p1.z);
...
bool Collision::rayplane(const float& nx, float ny, float nz, float x0, float y0, float z0, float xs, float ys, float zs, float xd, float yd, float zd, Vector3 p1, Vector3 p2, Vector3 p3, Vector3 p4, float* dis, Vector3* point)
{
	...
	float x = xs + t * xd;	//calculate the 3 point Vector3
	float y = ys + t * yd;
	float z = zs + t * zd;
	Vector3 i(x, y, z);
...
bool Collision::sphereplane(Vector3& sp, Vector3 pn, Vector3 p1, Vector3 p2, Vector3 p3, Vector3 p4, float r)
{
...
bool Collision::spheresphere(Vector3& c1, float r1, Vector3& c2, float r2)
{
